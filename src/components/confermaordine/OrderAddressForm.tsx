...
type Badge = { type: "success" | "error"; message: string } | null;

type OrderAddressFormProps = {
  order: Order;
  formData: FormState;
  setFormData: (data: FormState) => void; // se passi direttamente setState, va bene anche React.Dispatch<React.SetStateAction<FormState>>
  parcelCount: number;
  setParcelCount: (val: number) => void;
  onSave: () => void;
  loading: boolean;
  badge?: Badge; // è nel type originale ma non usato qui; lo rendo opzionale per compat
};
...
export default function OrderAddressForm({
  ...
  const paymentStatusKey = (order?.payment_status || "").toLowerCase().trim();
  const paymentString =
    paymentLabel[paymentStatusKey as keyof typeof paymentLabel] ??
    ...
    <div className="bg-gray-50 rounded-2xl p-5 shadow space-y-3">
      ...
      <div className="flex gap-4 items-center">
        ...
        <input
          ...
          value={parcelCount}
          onChange={(e) =>
            setParcelCount(Math.max(1, parseInt(e.target.value, 10) || 1))
          }
        />
      </div>
