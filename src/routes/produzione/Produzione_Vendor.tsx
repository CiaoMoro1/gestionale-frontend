...
type StatoProduzione = (typeof STATI_PRODUZIONE)[number];
...
export type ProduzioneRow = {
  ...
  stato: string;
  stato_produzione: StatoProduzione | string;
  da_produrre: number;
  cavallotti: boolean;
  note?: string;
...
export default function ProduzioneVendor() {
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      filtrate = filtrate.filter((row) => matchAllWords(row.sku + " " + row.ean, queryWords));
    }
  ...
  const badgeStatoCounts = useMemo(
    () =>
    ...
    [allRows, radice, search]
  );

  const badgeTuttiStati = useMemo(
    () =>
      allRows.filter(
        (r) =>
          (!radice || r.radice === radice) &&
  ...
  const radiciDisponibili = useMemo(
    () =>
      Array.from(
        new Set(
          allRows
    ...
    [allRows, statoProduzione]
  );

  const badgeRadiceCounts = useMemo(
    () =>
      radiciDisponibili.map((r) => ({
        radice: r,
        count: allRows.filter(
          (row) => (!statoProduzione || row.stato_produzione === statoProduzione) && row.radice === r
        ).length,
      })),
    [allRows, radiciDisponibili, statoProduzione]
  );

  const sortedBadgeRadiceCounts = useMemo(
    () =>
      [...badgeRadiceCounts].sort((a, b) =>
        (a.radice ?? "").localeCompare(b.radice ?? "", "it", { sensitivity: "base" })
      ),
    [badgeRadiceCounts]
  );

  const badgeTutteRadici = useMemo(
    () =>
      allRows.filter(
        (row) =>
    ...
    [allRows, statoProduzione, search]
  );
  ...
  async function exportPDF(orderBy: "az" | "misura") {
    ...
    Object.keys(byRadice).forEach((rad) => {
      ...
      arr.sort((a, b) => {
        if (orderBy === "misura") {
          ...
          const [a1, a2] = parseMisura(misuraA);
          const [b1, b2] = parseMisura(misuraB);
    ...
    let firstTable = true;
    for (const rad of Object.keys(byRadice)) {
      if (!firstTable) doc.addPage();
      const startY = 15;

      doc.setFontSize(16);
      doc.setFont("helvetica", "bold");
      doc.text(`Radice: ${rad}`, 12, startY);
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          0: { cellWidth: 40, halign: "center", overflow: "visible" },
          1: { cellWidth: 80, overflow: "linebreak" },
          2: { cellWidth: 50, overflow: "ellipsize" },
          3: { cellWidth: 40, overflow: "ellipsize" },
          4: { cellWidth: 30, halign: "center", overflow: "visible" },
        },
        didDrawCell: (data: CellHookData) => {
          const rowRaw = data.row?.raw as [string, string, string, string, number] | undefined;
          if (data.column.index === 0 && rowRaw && rowRaw[2]) {
            const ean = rowRaw[2];
            const img = barcodeMap[ean];
            if (img) {
              data.doc.addImage(img, "PNG", data.cell.x + 2, data.cell.y + 2, 30, 10);
            }
          }
        },
      });
  ...
  async function patchProduzione(
    id: number,
    ...
    try {
      ...
      if (res.status === 403) {
        ...
        const msg = data.error || "Password errata.";
  ...
  async function openMovimentiLog(produzioneId: number) {
    ...
    const data = (await res.json()) as LogMovimento[] | { data?: LogMovimento[] };
    const arr = Array.isArray(data) ? data : data?.data ?? [];
    setLogMovimentiOpen({ id: produzioneId, data: arr });
  }
  ...
  async function azioneMassiva(stato: StatoProduzione | "Rimossi") {
  ...
  function openCavallottoPdf(sku: string, formato: string) {
    window.open(
      `${import.meta.env.VITE_API_URL}/api/cavallotto/html?sku=${encodeURIComponent(
        sku
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto px-2 pt-6 pb-12 font-sans">
      ...
      {selezionati.length > 0 && (
        <div className="sticky top-2 z-40 flex flex-wrap items-center gap-3 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl px-6 py-3 mb-4 border morph-glass animate-fade-in">
          <span className="text-lg font-bold text-cyan-800 flex items-center gap-2">
            ...
            <span className="animate-badge-pop">{selezionati.length}</span> selezionat
            {selezionati.length === 1 ? "o" : "i"}
          </span>
        ...
        <div className="relative" ref={radiciBoxRef}>
          ...
          {radiciOpen && (
            ...
            >
              ...
              {sortedBadgeRadiceCounts.map(({ radice: r, count }) => (
                <div
                  key={r}
                  className={`px-3 py-2 hover:bg-cyan-50 cursor-pointer text-base rounded-xl font-semibold ${
      ...
      {/* Top scrollbar (sync) */}
      <div ref={scrollTopRef} className="overflow-x-auto mb-1 hidden sm:block">
        <div style={{ width: 1200, height: 1 }} />
      </div>
            ...
            {righeDaMostrare.map((r) => (
              <tr key={r.id} className="border-b border-gray-100 hover:bg-cyan-50/40 transition-all">
                    ...
                    <span className="text-xs text-gray-400 font-medium italic">
                      {(() => {
                        const allOfThis = allRows.filter(
                          (rr) => rr.sku === r.sku && rr.ean === r.ean && rr.start_delivery === r.start_delivery
                        );
                        const statoOrder: StatoProduzione[] = [
                          "Stampato",
                          "Calandrato",
                          "Cucito",
                          "Confezionato",
                        ...
                        const parti: Array<string | ReactElement> = [];
                        statoOrder.forEach((st) => {
                          ...
                          if (sum > 0) {
                            ...
                            parti.push(<span key={st}>{sum} {statoLabel[st]}</span>);
                          }
                        });
                        ...
                        if (daStampareEffettivo > 0) {
                          ...
                          parti.push(
                            <span key="ds" className="text-blue-900 font-bold">
                              {daStampareEffettivo} Da Stampare
                            </span>
                          );
                        }
                        if (plus > 0) {
                ...
                <td className="px-3 py-2 text-center" style={{ minWidth: 160 }}>
                  <span className="inline-block cursor-pointer" tabIndex={0} onClick={() => setStatoProduzioneOpenId(r.id)}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur">
          <div className="bg-white/90 rounded-2xl p-6 shadow-xl border max-w-xs w-full morph-glass relative animate-fade-in">
            ...
            <div className="flex gap-2 mt-4 justify-between">
              <button className="px-4 py-2 bg-gray-200 text-gray-700 rounded-xl font-bold hover:bg-gray-300" onClick={() => setModaleDaProdurre(null)}>
                Annulla
              </button>
              <button
          ...
          return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
              <div className="bg-white rounded-2xl p-6 shadow-2xl border flex flex-col w-full max-w-xs animate-fade-in relative">
                <button className="absolute top-2 right-3 text-neutral-400 hover:text-black text-2xl" onClick={() => setStatoProduzioneOpenId(null)}>
                  ×
                </button>
      ...
      {modaleNota && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-xs relative">
            <button className="absolute top-2 right-2 text-2xl text-gray-400 hover:text-black" onClick={() => setModaleNota(null)}>
              ×
            </button>
      ...
      {logMovimentiOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white/90 rounded-2xl p-6 shadow-2xl border w-full max-w-3xl morph-glass animate-fade-in relative">
            <button className="absolute top-2 right-4 text-neutral-400 hover:text-black text-2xl" onClick={() => setLogMovimentiOpen(null)}>
              ×
            </button>
              ...
              <table className="w-full text-[15px]">
                ...
                <tbody>
                  {logMovimentiOpen.data.length === 0 && (
                    <tr>
                      <td colSpan={6} className="text-gray-400 text-center py-10">
                        Nessun movimento registrato.
                      </td>
                    </tr>
                  )}
                  {logMovimentiOpen.data.map((log, i) => (
                    <tr key={`${log.created_at ?? ""}-${i}`} className={i % 2 ? "bg-gray-50" : ""}>
                      ...
                      <td className="py-1">
                        <span
                          className={`inline-block px-2 py-0.5 rounded-full text-xs font-bold ${
                            log.motivo?.toLowerCase().includes("stato")
                              ? "bg-cyan-100 text-blue-800"
                              : log.motivo?.toLowerCase().includes("quantità")
      ...
      {cavallottoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-xs text-center relative">
            <button className="absolute top-2 right-3 text-2xl text-gray-300 hover:text-black" onClick={() => setCavallottoModal(null)}>
              ×
            </button>
