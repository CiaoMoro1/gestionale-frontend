// src/routes/seller/PrelievoSeller.tsx

import { useEffect, useRef, useState } from "react";
import { ChevronRight } from "lucide-react";

import { supabase } from "../../lib/supabase";
import PrelievoModalSeller, {
  type SellerPrelievoRow,
} from "./PrelievoModalSeller";
...
function radiceFromRow(r: SellerPrelievoRow): string {
...
/* ===========================
   Pagina Prelievo Amazon Seller
=========================== */
export default function PrelievoSeller() {
  const API = import.meta.env.VITE_API_URL as string;

  const [prelievi, setPrelievi] = useState<SellerPrelievoRow[]>([]);
  const [allPrelieviData, setAllPrelieviData] =
    useState<SellerPrelievoRow[]>([]);
  const [modaleArticolo, setModaleArticolo] =
    useState<SellerPrelievoRow | null>(null);
  ...
  // Carica TUTTI i prelievi SELLER + ordini seller in picking
  useEffect(() => {
    (async () => {
      // 1) prelievi Seller (SKU aggregati)
      try {
        const res = await fetch(`${API}/api/prelievi-seller`);
        if (!res.ok) {
          setPrelievi([]);
          setAllPrelieviData([]);
        } else {
          const lista = (await res.json()) as SellerPrelievoRow[];
      ...
      // 2) ordini SELLER in picking (Supabase)
      try {
  ...
  // --- Aggiorna riscontro rapido (Seller usa stesso schema Sito) ---
  async function setRiscontroRapido(row: SellerPrelievoRow, nuovoRiscontro: number) {
    setPrelievi((prev) =>
      prev.map((r) => {
        if (r.id !== row.id) return r;

        const qty = r.qty;
        let nuovoStato: SellerPrelievoRow["stato"];
    ...
    try {
      const res = await fetch(`${API}/api/prelievi-seller/${row.id}`, {
        method: "PATCH",
  ...
  // --- ProgressBar (copiata vendor/sito) ---
  ...
  // --- Apertura modale ---
  function openModaleArticolo(row: SellerPrelievoRow) {
    setModaleArticolo(row);
  }

  // --- Refresh liste dopo salvataggio modale / processa ---
  async function refreshListe() {
    // ricarica prelievi seller
    const urlAll = `${API}/api/prelievi-seller`;
    const res1 = await fetch(urlAll);
    const lista1 = (await res1.json()) as SellerPrelievoRow[];
    ...
    // ricarica ordini seller in picking
  ...
  // --- Azioni massive (Seller, stessa UX del Sito) ---
  ...
  async function completaPending() {
    try {
      ...
      const tuttiPrelievi =
        (await resPrelievi.json()) as SellerPrelievoRow[];
  ...
  async function processaOrdini() {
    if (
      !window.confirm(
        "Vuoi processare gli ordini Amazon Seller in prelievo in base ai riscontri?"
      )
    )
      return;
    ...
    try {
      ...
      const res = await fetch(
        `${API}/api/prelievi-seller/processa`,
        { method: "POST" }
      );
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errJson?.error ||
            "Errore nel processamento ordini seller."
        );
      }
      ...
      setToast({
        msg: "Ordini Amazon Seller processati correttamente.",
        type: "success",
      });
    } catch (err) {
      setToast({
        msg:
          err instanceof Error
            ? err.message
            : "Errore durante il processamento ordini seller.",
        type: "error",
      });
      setProgressActive(false);
    }
  ...
  async function svuotaLista() {
    if (
      !window.confirm(
        "Sei sicuro di voler svuotare tutta la lista prelievi Seller? Operazione IRREVERSIBILE!"
      )
    )
      return;
    await fetch(`${API}/api/prelievi-seller/svuota`, {
      method: "DELETE",
    });
    setPrelievi([]);
    setAllPrelieviData([]);
    setOrders([]);
    setToast({
      msg: "Lista prelievi Seller svuotata!",
      type: "success",
    });
  }
      ...
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        ...
        <div className="max-w-md w-full bg-white rounded-xl p-8 shadow text-center flex flex-col items-center gap-4">
          <div className="text-2xl font-bold mb-2 text-blue-800">
            Nessun articolo in prelievo (Seller)
          </div>
          <div className="text-sm text-gray-500">
            Seleziona ordini dalla pagina Ordini Amazon Seller e
  ...
  return (
    <div className="w-full max-w-[1100px] mx-auto px-1 pb-10 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3 justify-between w-full pt-2">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-semibold text-base text-blue-900 truncate">
              Prelievo Amazon Seller
            </span>
            <span className="text-xs text-neutral-500 truncate">
              Articoli attualmente in prelievo (canale Amazon Seller)
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 ml-auto">
          ...
          <button
            ...
            aria-label="Processa ordini seller"
          >
            ⚙️ Processa ordini
          </button>
          ...
          <button
            ...
            aria-label="Svuota lista prelievi seller"
          >
            🗑️ Svuota lista
          </button>
        </div>
      ...
      {/* Tabella prelievi SELLER (righe SKU aggregate) */}
          ...
          <table className="w-full min-w-[820px] text-[15px] sm:text-[17px]">
                ...
                <th className="px-2 py-2 text-center w-[44px]">
                  <input
                    ...
                    checked={
                      ...
                      selectedIds.length === prelieviToShow.length
                    }
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedIds(
            ...
            <tbody>
              {prelieviToShow.map((r) => {
                const processed =
                  r.riscontro !== null && r.stato !== "in verifica";
      ...
      {/* Tabella ORDINI Seller in picking */}
          ...
          <tbody>
              ...
              >
                ...
                <td className="p-3 text-center">
                  <button
                    ...
                    onClick={async () => {
                      // 1) rimetti l'ordine a NUOVO lato Seller
                      await supabase
                        .from("orders_seller")
                        .update({
                          stage: "NUOVO",
            ...
            {orders.length === 0 && (
              <tr>
                ...
                >
                  Nessun ordine Amazon Seller in prelievo.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <PrelievoModalSeller
        ...
        onSaved={async () => {
          await refreshListe();
          setToast({
            msg: "Prelievo Seller aggiornato!",
            type: "success",
          });
        }}
      />
