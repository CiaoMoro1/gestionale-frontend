// src/routes/seller/PrelievoModalSeller.tsx
...
/** === Tipi per SELLER, copiati da Sito === */
export type SellerPrelievoRow = {
  id: number;
...
/* ==============================
   Modale principale: Prelievo SELLER
============================== */
export default function PrelievoModalSeller({
  open,
  row,
  onClose,
  onSaved,
  ...
  const [cavallottoModalSku, setCavallottoModalSku] =
    useState<string | null>(null);
  const [cavallottoLoading, setCavallottoLoading] =
  ...
  useEffect(() => {
    ...
    // Per SELLER il default di magazzino_usato è sul canale "Amazon Seller"
    const prev = row.mag_usato_by_canale ?? {
      "Amazon Vendor": 0,
      Sito: 0,
      "Amazon Seller": n(row.magazzino_usato),
    };
        ...
        <div className="w-[97%] max-h-[90%] sm:w-[50%] top-1 overflow-y-auto bg-white rounded-2xl shadow-lg border relative">
          ...
          <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b px-5 py-3 flex items-center justify-between">
            <div className="font-bold text-blue-700 text-lg">
              Modifica Prelievo Amazon Seller
            </div>
            <button
